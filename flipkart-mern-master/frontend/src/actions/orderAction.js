...
export const getPaymentStatus = (id) => async (dispatch) => {
    try {
        ...
        const { data } = await axios.get(`http://localhost:3000/api/v1/payment/status/${id}`);
...
export const updateOrder = (id, order) => async (dispatch) => {
    try {
        ...
        const { data } = await axios.put(`http://localhost:3000/api/v1/admin/order/${id}`, order, config);
