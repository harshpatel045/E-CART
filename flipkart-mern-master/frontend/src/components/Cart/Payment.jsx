...
const Payment = () => {
    ...
    const initPayment = (data)=>{
        const options = {
            ...
            handler: async (res)=>{
                try{
                    const verifyUrl = "http://localhost:3000/api/v1/payment/callback";
    ...
    console.log()
    try{
        const orderUrl = "http://localhost:3000/api/v1/payment/process";
        const {data} = await axios.post(orderUrl, {amount: paymentData.amount});
        console.log(data);
        const orderData = {
            shippingInfo,
            orderItems: cartItems,
